...
//       // For each leave, fetch messages to check manager approval
//       const leavesWithStatus = await Promise.all(
//         response.data.map(async (leave) => {
//           try {
//             const messagesResponse = await axios.get(
//               `http://localhost:5000/api/leaves/${leave.id}/messages`,
//               { headers: { Authorization: `Bearer ${token}` } }
//             );

//             // Check if any message contains "Manager Approved"
//             const managerApproved = messagesResponse.data.some(
//               msg => msg.body && msg.body.includes('Manager Approved')
//             );
...
//             };
//           } catch (err) {
//             return { ...leave, managerApproved: false };
//           }
//         })
//       );
...
//               <TableCell><strong>Manager Status</strong></TableCell> {/* NEW */}
...
const PendingLeavesTable = ({ onActionComplete }) => {
  ...
  const fetchPendingLeaves = async () => {
    try {
      ...
      const allPendingLeaves = [
        ...confirmLeaves.data,
        ...hrPendingLeaves.data
      ];

      setLeaves(allPendingLeaves);
      setError('');
    } catch (err) {
      setError('Failed to load pending leaves');
      console.error(err);
  ...
  const getStatusDisplay = (state) => {
    const statusMap = {
      'confirm': { label: 'Pending Manager', color: 'warning' },
      'validate1': { label: 'Pending HR', color: 'info' },
      'validate': { label: 'Approved', color: 'success' },
      'refuse': { label: 'Rejected', color: 'error' }
    };

    return statusMap[state] || { label: state, color: 'default' };
  };
        ...
        <Table>
          ...
          <TableBody>
            {leaves.length === 0 ? (
              ...
              leaves.map((leave) => {
                const status = getStatusDisplay(leave.state);
                return (
                  <TableRow key={leave.id}>
                    <TableCell>{leave.employee_id[1]}</TableCell>
                    <TableCell>
